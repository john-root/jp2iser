...
CMD_COMPRESS = {
    "kdu_low":  '{kdu} -i {input} -o {output} Clevels=7 "Cblk={{64,64}}" "Cuse_sop=yes" -jp2_space sRGB  ' +
            '"ORGgen_plt=yes" "ORGtparts=R" "Corder=RPCL" -rate 0.5 ' +
            '"Cprecincts={{256,256}},{{256,256}},{{256,256}},{{128,128}},' +
            '{{128,128}},{{64,64}},{{64,64}},{{32,32}},{{16,16}}"',
    "kdu_med":  '{kdu} -i {input} -o {output} Clevels=7 "Cblk={{64,64}}" "Cuse_sop=yes" -jp2_space sRGB  ' +
            '"ORGgen_plt=yes" "ORGtparts=R" "Corder=RPCL" -rate 2 ' +
            '"Cprecincts={{256,256}},{{256,256}},{{256,256}},{{128,128}},' +
            '{{128,128}},{{64,64}},{{64,64}},{{32,32}},{{16,16}}"',
    "kdu_high": '{kdu} -i {input} -o {output} Clevels=7 "Cblk={{64,64}}" "Cuse_sop=yes" -jp2_space sRGB  ' +
            '"ORGgen_plt=yes" "ORGtparts=R" "Corder=RPCL" -rate 4 ' +
            '"Cprecincts={{256,256}},{{256,256}},{{256,256}},{{128,128}},' +
            '{{128,128}},{{64,64}},{{64,64}},{{32,32}},{{16,16}}"',
    "kdu_max":  '{kdu} -i {input} -o {output} Clevels=7 "Cblk={{64,64}}" "Cuse_sop=yes" -jp2_space sRGB  ' +
